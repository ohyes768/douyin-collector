...
### 1.1 背景
从现有 `douying-collect` 项目迁移代码，创建一个简化的 Windows 端视频采集工具，负责采集抖音收藏视频并上传到服务器。

### 1.2 目标
- **核心目标**：采集抖音收藏视频 → 下载 → 上传到 file-system-go 服务器
- **附加目标**：支持时间过滤、增量处理、自动重试
- **非目标**：不做 ASR 识别、不做音频处理（由 douyin-processor 负责）
...
| 视频下载 | P0 | 下载视频文件到本地缓存 |
| 文件上传 | P0 | 上传到 file-system-go 服务器 |
| 时间过滤 | P1 | 默认采集最近 7 天的视频 |
| 增量处理 | P1 | 检查服务器是否已存在，跳过重复文件 |
| 自动重试 | P1 | 失败自动重试 3 次 |
