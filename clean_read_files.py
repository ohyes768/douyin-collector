...
"""
已读文件清理程序
定期清理 file-system-go 中超过指定天数的已读文件
"""
...
from src.read_files_cleaner import ReadFilesCleaner
...
async def main():
    ...
    logger.info("=" * 60)
    logger.info("已读文件清理程序启动")
    logger.info("=" * 60)

    # 获取清理配置
    cleanup_config = config["app"].get("read_files_cleanup", {})
    if not cleanup_config.get("enabled", False):
        logger.info("已读文件清理功能未启用，退出")
        sys.exit(0)
    ...
    cleaner = ReadFilesCleaner(server_url, days_threshold)

    # 执行清理
    try:
        result = await cleaner.clean_read_files()
